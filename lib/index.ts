...
class DrawingSurface {
  ...
  private execute(
    ...
    if (spec.line) {
      ...
      spec.turtles.forEach((turtleOp) => {
        if (turtleOp.op === "name" && turtleOp.name) {
          ...
          if (typeof turtleOp.turn === "number") {
            ...
            if (turtleOp.turn.degrees !== undefined) {
              angleRadians = degreesToRadians(turtleOp.turn.degrees);
        ...
        console.log(`After turtle op ${turtleOp.op} the turtleXfm is now:`);
  ...
  drawTurtleCursor(
    ...
    const baseCorner1 = {
      x: position.x - direction.dx * (size / 2) + perpendicular.dx * (size / 2),
      y: position.y - direction.dy * (size / 2) + perpendicular.dy * (size / 2),
    };
